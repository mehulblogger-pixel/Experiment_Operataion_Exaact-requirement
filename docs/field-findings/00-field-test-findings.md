...
| # | Type | Sev | Finding | Status |
|---|---|---|---|---|
| 22 | 🐞 | **P0** | **INSPECTOR SCOPE LEAK.** A call allocated to **Mr. Vijay only** is also visible in **Mr. Ravi's schedule**. Inspectors are not scoped to their own allocations. *(Highest priority — a segregation defect.)* | ☐ |
| 12 | ❓ | P1 | **Cross-office scope question.** If Mr. X (inspector) / an Ahmedabad coordinator handles a call for a **Mumbai-office** client **executed in Ahmedabad**, should they see the call details? Define the expected rule, then verify. | ☐ |
| 26 | 🐞 | P1 | **Second inspector / contractor (Ravi) not reflected** — not shown on the operational dashboard, during job allocation, etc. | ☐ |
