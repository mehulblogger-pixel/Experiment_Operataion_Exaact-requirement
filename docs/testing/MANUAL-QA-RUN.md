...
| D-003 | 6 | **P2 (multi-tenant confidentiality / onboarding)** | `auto_seed()` loads `data/seed_data.json` (**327 real client + 452 real vendor names**) into ANY fresh DB; not gated to the control install | `lib/db.php` `auto_seed()` (called from `run_schema`) | A brand-new customer/tenant workspace starts **empty** of partners | Every fresh DB (incl. a new tenant workspace) boots with **779 real MGH client/vendor names** | ✅ **FIXED** (owner chose "Fix now"). A fresh **real hosted** install (fpm/apache SAPI) now starts **empty**; the bundled sample list loads only on explicit opt-in (`seed_sample_partners=1`) or the master-only **"Load sample data"** button on an empty Clients list. Dev/CLI + the local server still seed, so tests & the crawl are unchanged; the owner's populated live site is untouched (early-returns on `$n>0`). New `auto_seed_load_sample()` + `/load-sample-data` route + 9 tests; full harness green. |
