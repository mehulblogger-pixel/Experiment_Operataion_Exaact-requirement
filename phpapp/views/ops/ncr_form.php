...
<form method="post" action="/ncr-new" class="panel" style="margin-top:16px;max-width:820px">
  <div class="form-grid" style="gap:12px 16px">
    <div class="ff-wide"><label>What was wrong <span class="muted">(one line)</span></label>
      <input name="title" required value="<?= e($p['title'] ?? '') ?>" placeholder="e.g. Report issued naming an instrument that was out of calibration"></div>
    ...
    <div><label>Reference at the source <span class="muted">(optional)</span></label>
      <input name="source_note" value="<?= e($p['source_note'] ?? '') ?>" placeholder="e.g. the client's e-mail, the finding number"></div>
    ...
    <div><label>Detected on</label><input type="date" name="detected_on" value="<?= e($p['detected_on'] ?? date('Y-m-d')) ?>"></div>
    <div><label>Clause <span class="muted">(optional)</span></label><input name="clause" value="<?= e($p['clause'] ?? '') ?>" placeholder="e.g. 6.2.6"></div>

    <div><label>Owner <span class="muted">(who will see it through)</span></label>
      <input name="owner" value="<?= e($p['owner'] ?? '') ?>" placeholder="name or e-mail"></div>
    <div><label>Due by <span class="muted">(blank = 14 days if major, 30 if not)</span></label>
      <input type="date" name="due_on" value="<?= e($p['due_on'] ?? '') ?>"></div>

    <div class="ff-wide"><label>What was done immediately <span class="muted">(can be added later)</span></label>
      <textarea name="containment" rows="2" placeholder="e.g. the report was recalled from the client the same day"><?= e($p['containment'] ?? '') ?></textarea></div>
  </div>
