...
<form method="get" action="/inquiries" class="filter-bar" style="gap:8px;flex-wrap:wrap">
  ...
  <select class="form-control" name="st" onchange="this.form.submit()"><option value="">All statuses</option>
    <?php foreach (INQUIRY_STATUS as $k=>$v): ?><option value="<?= e($k) ?>" <?= $st===$k?'selected':'' ?>><?= e($v) ?></option><?php endforeach; ?></select>
...
<div class="panel" style="padding:0;overflow:hidden">
  ...
  <table class="dt">
    ...
    <tr>
      ...
      <td><?= e(INQUIRY_SOURCES[$r['source']] ?? $r['source']) ?></td>
      <td><span class="pill <?= $stPill[$r['status']] ?? 'p-mut' ?>"><?= e(INQUIRY_STATUS[$r['status']] ?? $r['status']) ?></span></td>
