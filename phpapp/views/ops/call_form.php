...
<form method="post" action="<?= $call ? '/call-edit?id=' . (int)$call['id'] : '/call-new' ?>" class="panel">
  <div class="form-grid">
    ...
    <div class="ff"><label>Region</label>
      <select class="form-control" name="region"><option value="">—</option>
        <?php foreach (OPS_REGIONS as $k=>$v): ?><option value="<?= $k ?>" <?= ($call && $call['region']===$k)?'selected':'' ?>><?= e($v) ?></option><?php endforeach; ?>
      </select></div>
    <div class="ff"><label>SBU</label>
      <select class="form-control" name="sbu"><option value="">—</option>
        <?php foreach (OPS_SBUS as $k=>$v): ?><option value="<?= $k ?>" <?= ($call && $call['sbu']===$k)?'selected':'' ?>><?= e($v) ?></option><?php endforeach; ?>
      </select></div>
    <div class="ff"><label>Product category</label>
      <select class="form-control" name="product_category"><option value="">—</option>
        <?php foreach (PRODUCT_CATS as $k=>$v): ?><option value="<?= $k ?>" <?= ($call && $call['product_category']===$k)?'selected':'' ?>><?= e($v) ?></option><?php endforeach; ?>
      </select></div>
