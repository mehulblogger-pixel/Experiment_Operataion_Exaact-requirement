...
<form method="post" action="/lead-new" class="panel" style="margin-top:16px;max-width:860px">
  <div class="form-grid" style="gap:12px 16px">
    <div class="ff-wide"><label>Company or person *</label>
      <input class="form-control" name="company_name" required maxlength="200" value="<?= e($p['company_name'] ?? '') ?>"
             placeholder="Who are we chasing?"></div>
    <div><label>Pipeline</label><select class="form-control" name="pipeline_id">
      <?php foreach ($pipelines as $pl): ?><option value="<?= (int)$pl['id'] ?>"><?= e($pl['name']) ?></option><?php endforeach; ?>
    </select></div>
    <div><label>Where they came from</label><select class="form-control" name="source">
      <option value="">—</option>
      <?php foreach ($sources as $k=>$v): ?><option value="<?= e($k) ?>"><?= e($v) ?></option><?php endforeach; ?>
    </select></div>
    <div><label>Contact name</label><input class="form-control" name="contact_name" maxlength="150" value="<?= e($p['contact_name'] ?? '') ?>"></div>
    <div><label>Branch</label><select class="form-control" name="office_id"><option value="">— mine —</option>
      <?php foreach ($offices as $o): ?><option value="<?= (int)$o['id'] ?>"><?= e($o['name']) ?></option><?php endforeach; ?>
    </select></div>
    <div><label>E-mail</label><input class="form-control" type="email" name="contact_email" maxlength="200" value="<?= e($p['contact_email'] ?? '') ?>"></div>
    <div><label>Telephone</label><input class="form-control" name="contact_phone" maxlength="60" value="<?= e($p['contact_phone'] ?? '') ?>"></div>
    <div class="ff-wide"><label>What do they want?</label>
      <textarea class="form-control" name="requirement" rows="3" placeholder="Scope, quantities, the standard, anything that decides whether this is worth chasing"><?= e($p['requirement'] ?? '') ?></textarea></div>
    <div><label>Value they are worth <span class="muted">(estimate)</span></label><input class="form-control" type="number" step="0.01" min="0" name="value" value="<?= e($p['value'] ?? '') ?>"></div>
    <div><label>Expected to close</label><input class="form-control" type="date" name="expected_close" value="<?= e($p['expected_close'] ?? '') ?>"></div>
    <div><label>Next thing to do</label><input class="form-control" name="next_action" maxlength="255" placeholder="e.g. send the capability statement"></div>
