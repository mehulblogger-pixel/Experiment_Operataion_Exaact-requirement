...
<p class="plead" style="text-align:center;margin-left:auto;margin-right:auto">
  You have been invited to the <?= e(app_name()) ?> vendor portal. Choose a password now. Nobody here will ever
...
<?php if (!empty($dead)): ?>
  <p class="pnote" style="max-width:430px;margin:22px auto 0;text-align:center">
    ...
    <a href="/vendor/login">Sign in</a> if you have already set a password.</p>
<?php else: ?>
<form class="pform" method="post" action="/vendor/accept">
  <input type="hidden" name="t" value="<?= e($token ?? '') ?>">
  <label for="vapass">New password</label>
  <input class="form-control" id="vapass" name="password" type="password" autocomplete="new-password" required autofocus>
  <label for="vaconf">Type it again</label>
  <input class="form-control" id="vaconf" name="confirm" type="password" autocomplete="new-password" required>
