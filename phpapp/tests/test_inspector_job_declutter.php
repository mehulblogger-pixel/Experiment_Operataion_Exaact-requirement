...
// It keys off the field-inspector predicate (INSPECTOR or SR_INSPECTOR), falling
// back to is_inspector() only if the helper is somehow unavailable.
t_ok(strpos($src, '$fieldInspector = function_exists(\'is_field_inspector\') ? is_field_inspector()') !== false,
    'the gate is the field-inspector predicate (INSPECTOR / SR_INSPECTOR)');
...
$pdo->prepare("INSERT INTO users (username, first_name, role, is_active) VALUES ('ijd_sr','Sr','SR_INSPECTOR',1)")->execute();
$srUid = (int)$pdo->lastInsertId();

$fieldInspector = fn() => is_field_inspector();

$_SESSION['uid'] = $insUid; current_user(true); ua(true);
t_ok($fieldInspector() === true, 'an inspector gets the stripped view');
$_SESSION['uid'] = $srUid; current_user(true); ua(true);
t_ok($fieldInspector() === true, 'a senior inspector also gets the stripped field view (R7)');
