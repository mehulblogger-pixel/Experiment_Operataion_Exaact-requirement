...
function run_schema($withSeeds = true) {
    ...
    if (function_exists('mkt_escrow_migrate')) mkt_escrow_migrate();              // Marketplace escrow lifecycle (hold → release/refund)
    if (function_exists('mkt_rules_migrate')) mkt_rules_migrate();                // Compliance rule master (versioned, effective-dated)
    ...
    if (function_exists('cockpit_migrate')) cockpit_migrate();         // Company Setup Cockpit — business capabilities
    if (function_exists('saas_entitlement_ensure')) saas_entitlement_ensure(); // grandfather existing tenants onto their paid module ceiling
    ...
    if (function_exists('pwreset_migrate')) pwreset_migrate();         // self-service forgot/reset password tokens
