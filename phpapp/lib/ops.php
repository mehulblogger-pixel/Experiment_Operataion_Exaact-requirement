...
function ops_module_gate($route, $peek = false) {
    ...
    static $map = [
        ...
        'opportunity-raise-order'=>'leads','opportunity-send-to-accounts'=>'leads','opportunity-contract'=>'leads','opportunity-generate-quote'=>'leads',
...
function ops_dispatch($route, $method) {
    ...
    switch (true) {
        ...
        case $route === 'quotes' || $route === 'quote' || $route === 'quote-new' || $route === 'quote-edit' || $route === 'quote-revise' || $route === 'quote-status' || $route === 'quote-doc' || $route === 'quote-pdf' || $route === 'quote-approve' || $route === 'quote-unapprove' || $route === 'quote-contract' || $route === 'quote-add-contract' || $route === 'quote-float' || $route === 'quote-client' || $route === 'partner-address' || $route === 'quote-files' || $route === 'quote-file' || $route === 'quote-file-delete' || $route === 'quote-unlock' || $route === 'quote-followup' || $route === 'quote-external' || $route === 'quotes-export' || $route === 'quote-final' || $route === 'quote-compose' || $route === 'followup-compose' || $route === 'quote-preorder-save':
        ...
        case $route === 'recruit-export':                            // Phase 7 — recruitment CSV exports
        ...
        case $route === 'jd-generate':                               // auto job-description / posting (AJAX)
            return ops_jd_generate($route, $method);
        case $route === 'cert-add':                                  // learn a new certificate (AJAX)
            return ops_cert_add($route, $method);
        ...
        case $route === 'marketplace-toggle':                        // one-tap Marketplace on/off
            return ops_marketplace_toggle($method);
        ...
        case in_array($route, ['documents','document','document-new','document-edit','document-submit','document-finalize','document-delete','release-notes','document-ai-review','document-rn-flag'], true):
        ...
        case $route === 'document-vet-review':
            return ops_idems_vet_review($method);
        case $route === 'vetting-checklist':
            return ops_idems_vetting_checklist($method);
        ...
        case $route === 'document-polish-text':
            return ops_idems_polish_text($method);
...
function ops_inspectors($action, $method) {
    ...
    if ($action === 'new' || $action === 'edit') {
        ...
        if ($method === 'POST') {
            ...
            // The agency this person is engaged through. Picked from the agencies
            // master (a real link), with the name kept alongside so existing
            // displays and cost logic that read agency_name keep working. A typed
            // name is still honoured when no agency is chosen, so nothing a
            // coordinator entered before is lost.
            $agencyId = ($b['agency_id'] ?? '') !== '' ? (int)$b['agency_id'] : null;
            $agencyName = $b['agency_name'] ?? '';
            if ($agencyId) { $ag = agency_get($agencyId); if ($ag) $agencyName = (string)$ag['name']; }
            ...
            if ($ins) {
                $sql = "UPDATE inspectors SET first_name=?,middle_name=?,last_name=?,name=?,emp_code=?,designation=?,staff_kind=?,trade_id=?,sbus=?,sbu=?,skill_ids=?,email=?,mobile=?,agency_id=?,agency_name=?,home_office_id=?,weekly_working_days=?,reports_to_id=?,team_role=?,status=?";
                $args = [$b['first_name'] ?? '', $b['middle_name'] ?? '', $b['last_name'] ?? '', $full, $empCode, $desig, $kind, $trade, $sbus, explode(',', $sbus)[0] ?? '', $skills, $b['email'] ?? '', $b['mobile'] ?? '', $agencyId, $agencyName, $homeOff, $wwd, $reportTo, $teamRole, $b['status'] ?? 'ACTIVE'];
