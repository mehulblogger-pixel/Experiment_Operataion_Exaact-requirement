...
function setting_meta_all() {
    return [
        ...
        'revenue_reader_mode'        => ['Revenue reader mode (§28 / P9)', 'Which figure the revenue readers show for a job\'s invoiced amount as the books ledger becomes the source of truth. “reconciled” (the default) uses the ledger only where it agrees with the legacy per-job snapshot within tolerance, so it changes no number that has not been proven equal; “ledger” is the full switch (turn on once /revenue-reconciliation reads green); “legacy” is the pre-switch snapshot. Never destroys the legacy figure. Set from the revenue reconciliation screen.', ['Management dashboard (Module 34)'], 'live', 'high'],
