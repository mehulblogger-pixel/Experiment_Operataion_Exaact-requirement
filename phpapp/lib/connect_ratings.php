...
function cx_rating_add($requirementId, $direction, array $in) {
    ...
    db()->prepare("INSERT INTO cx_ratings
        (requirement_id,application_id,direction,rater_party_id,ratee_inspector_id,ratee_party_id,
         stars,competency,communication,punctuality,professionalism,would_rehire,comment,created_by,created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
        ->execute([$requirementId, (int)($in['application_id'] ?? 0), $direction,
                   ...
                   !empty($in['would_rehire']) ? 1 : 0, trim((string)($in['comment'] ?? '')),
...
/** Marketplace rating summary for a professional: [count, avg_stars, rehire_pct]. */
function cx_rating_summary_for_inspector($inspectorId) {
    connect_ratings_migrate();
    $inspectorId = (int)$inspectorId;
    $rows = ops_all("SELECT stars, would_rehire FROM cx_ratings WHERE ratee_inspector_id=? AND direction='CLIENT_TO_PRO'", [$inspectorId]) ?: [];
    $n = count($rows);
