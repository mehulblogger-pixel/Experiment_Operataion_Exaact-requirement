...
<form method="post" action="/opportunity-new" class="panel" style="margin-top:16px;max-width:900px">
  <div class="form-grid" style="gap:12px 16px">
    <div class="ff-wide"><label>What is the opportunity? *</label>
      <input name="name" required maxlength="255" value="<?= e($prefill['name'] ?? '') ?>"
    ...
    <div><label>…or who it is for</label>
      <input name="partner_name" maxlength="200" value="<?= e($prefill['partner_name'] ?? '') ?>"
    ...
    <div><label>Estimated value</label><input name="value" type="number" step="0.01" value="<?= e($prefill['value'] ?? '') ?>" placeholder="0.00">
    ...
    <div><label>Expected close</label><input type="date" name="expected_close" value="<?= e($prefill['expected_close'] ?? '') ?>"></div>
    ...
    <?php endif; ?>
    <div><label>Competing against</label><input name="competitor" maxlength="200" placeholder="Who else is bidding, if you know"></div>
    <div><label>Contact</label><input name="contact_name" maxlength="150"></div>
    <div><label>Contact e-mail</label><input name="contact_email" type="email" maxlength="200"></div>
    <div><label>Contact phone</label><input name="contact_phone" maxlength="60"></div>
    <div><label>Next action</label><input name="next_action" maxlength="255" placeholder="e.g. Send the scope for review"></div>
    <div><label>By when</label><input type="date" name="next_action_on"></div>
    <div class="ff-wide"><label>What they need</label><textarea name="requirement" rows="3"></textarea></div>
  </div>
