...
<form method="post" action="<?= $req ? '/requisition-edit?id='.(int)$req['id'] : '/requisition-new' ?>" class="panel">
  <div class="form-grid">
    <div class="ff"><label>Type *</label><select class="form-control" name="req_type" id="rq_type"><?php foreach (lk_options_or('requisition_type', REQ_TYPES) as $k=>$v): ?><option value="<?= e($k) ?>" <?= ($req && $req['req_type']===$k)?'selected':'' ?>><?= e($v) ?></option><?php endforeach; ?></select></div>
    ...
    <div class="ff"><label><?= e(T("sbu")) ?></label><select class="form-control" name="sbu"><option value="">—</option><?php foreach (OPS_SBUS as $k=>$v): ?><option value="<?= e($k) ?>" <?= ($req && $req['sbu']===$k)?'selected':'' ?>><?= e($v) ?></option><?php endforeach; ?></select></div>
    ...
    <div class="ff"><label>Status</label><select class="form-control" name="status"><?php foreach (lk_options_or('requisition_status', REQ_STATUS) as $k=>$v): ?><option value="<?= e($k) ?>" <?= ($req ? $req['status']===$k : $k==='OPEN')?'selected':'' ?>><?= e($v) ?></option><?php endforeach; ?></select></div>
