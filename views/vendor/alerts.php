...
<p class="plead">What has happened on your account — a report shared, a nonconformity raised, a response we have
  reviewed. Newest first; unread are highlighted.</p>

<?php if ($rows): ?>
<form method="post" action="/vendor/alerts" style="margin:0 0 14px">
...
<div class="pscroll"><table class="ptable">
  ...
  <?php foreach ($rows as $r): ?>
    <tr style="<?= empty($r['is_read']) ? 'font-weight:600' : 'opacity:.7' ?>">
      ...
      <td><a href="<?= e($r['url'] ?: '/vendor') ?>"><?= e($r['title']) ?></a>
      ...
      <td><?php if (empty($r['is_read'])): ?>
        <form method="post" action="/vendor/alerts" style="display:inline">
